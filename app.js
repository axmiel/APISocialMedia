...
const myname =
  localStorage.getItem("myName") ?? window.prompt("Enter your username");
localStorage.setItem("myName", myname);
const mytag = localStorage.getItem("mytag") ?? window.prompt("Enter your tag");
localStorage.setItem("mytag", mytag);
const mypfp =
  localStorage.getItem("mypfp") ??
  window.prompt("Add a profile picture (from url)");
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  const [mappedPostData, mappedUserData] = filteredPostData.map(
    (settledValue) => {
...
const userContainerPublishButton = document.getElementById(
  "user-publish-button"
);
const timelineInnerPostsContainer = document.querySelector(
  ".tl-inner-posts-container"
);
const userProfilePosts = document.querySelector(".user-posts");

timelinePublishButton.addEventListener("click", async () => {
  ...
  const newPostData = await publishPost(postData);

  timelineEditBtn.addEventListener("click", () => {
    editPost(newPostData.id, {
      value: "We have updated the post",
    });
  });
  ...
  timelineSubPostItems.append(
    timelineEditBtn,
    timelineDeleteBtn,
    timelinePostDate
  );
  userCardSubPostItems.append(
    userCardEditBtn,
    userCardDeleteBtn,
    userCardPostDate
  );
  ...
  timelinePostContainer.append(
    timelineUserDetails,
    timelinePostContent,
    timelineSubPostItems
  );

  //construct timeline post content container
  timelinePostContentContainer.append(
    timelinePostUserPicture,
